...
class Autorizacion(FPDF):
   ...
   # idSolicitud=request.POST['idSoli']
    ...
    pdf.line(60, 150, 100, 150)
    pdf.text(x=20, y=165, txt='DUI: ')
    pdf.line(60, 165, 100, 165)
    pdf.text(x=65, y=164, txt=per.Dui if hasattr(per, 'Dui') else '')
    pdf.text(x=20, y=180, txt='Correo electrónico: ')
    pdf.line(60, 180, 120, 180)
    pdf.text(x=65, y=179, txt=per.Correo if hasattr(per, 'Correo') else '')
    pdf.text(x=20, y=195, txt='Número de teléfono: ')
    pdf.line(60, 195, 120, 195)
    pdf.text(x=65, y=194, txt=per.Telefono if hasattr(per, 'Telefono') else '')
    pdf.text(x=20, y=210, txt='Fecha: ')
    pdf.line(60, 210, 100, 210)
    pdf.text(x=65, y=209, txt=fecha.strftime("%d/%m/%Y") if fecha else '')
